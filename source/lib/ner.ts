...
function generateNamedEntities(podcasts: any[]): Promise<boolean> {
  const pdcsts = Promise.all(
    podcasts.slice(startIndex, endIndex).map(async (podcast, i) => {
      const parsedRssFeed: any = await ner(JSON.parse(podcast)).catch((error: any) => console.log('Error: ', error.message))
